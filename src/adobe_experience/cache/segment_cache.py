"""Segment ID cache for number-based selection.

Stores recent segment listings with numbered mappings to enable commands like:
    aep segment get 1
Instead of:
    aep segment get d8a68c9e-1d5f-4b6c-8a4e-9f8c7d6e5f4a
"""
...
class SegmentCache:
    """Manages cached segment ID mappings for numbered selection."""
    ...
    def __init__(self, ttl_minutes: int = 60):
        ...
        self.cache_file = self.cache_dir / "recent_segments.json"
    ...
    def save_mappings(self, mappings: Dict[int, str]) -> None:
        ...
        Args:
            mappings: Dictionary mapping numbers (1, 2, 3...) to segment IDs
        """
        cache_data = {
    ...
    def get_id_by_number(self, number: int) -> Optional[str]:
        """Retrieve segment ID by number.

        Args:
            number: Number from recent segment list (1-based index)

        Returns:
            Segment ID if found and not expired, None otherwise
        """
