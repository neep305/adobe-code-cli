"""Dataflow ID cache for number-based selection.

Stores recent dataflow listings with numbered mappings to enable commands like:
    aep dataflow failures 1
Instead of:
...
class DataflowCache:
    """Manages cached dataflow ID mappings for numbered selection."""
    ...
    def __init__(self, ttl_minutes: int = 60):
        ...
        self.cache_file = self.cache_dir / "recent_dataflows.json"
    ...
    def save_mappings(self, mappings: Dict[int, str]) -> None:
        ...
        Args:
            mappings: Dictionary mapping numbers (1, 2, 3...) to dataflow IDs
        """
        cache_data = {
    ...
    def get_id_by_number(self, number: int) -> Optional[str]:
        """Retrieve dataflow ID by number.

        Args:
            number: Number from recent dataflow list (1-based index)

        Returns:
            Dataflow ID if found and not expired, None otherwise
        """
